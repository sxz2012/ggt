% A self-contained rewrite of Office Hour One ("Groups"), following the idea flow of
% Clay & Margalit, Office Hours with a Geometric Group Theorist (2017).
% All prose, figures and problems here are written from scratch; no text is copied.
% Build:  tectonic chapter1_book.tex
...
\usepackage{pifont}
\usepackage{hyperref}
\hypersetup{colorlinks=true,linkcolor=big,citecolor=big,urlcolor=big}
...
\begin{tikzpicture}[scale=1.55]
  ...
  \draw[dashed,thick,gray!70!black] (90:1.75)--(270:1.75);
  \draw[dashed,thick,gray!70!black] (126:1.75)--(306:1.75);
  \node[font=\scriptsize,gray!70!black,fill=white,inner sep=1pt] at (90:1.95) {axis of $t$};
  \node[font=\scriptsize,gray!70!black,fill=white,inner sep=1pt] at (126:2.05) {axis of $s$};
  \draw[{Stealth[length=3pt]}-{Stealth[length=3pt]},thin,mkc] (90:0.62) arc (90:126:0.62);
  \node[font=\scriptsize,mkc,anchor=south east,fill=white,inner sep=1pt] at (108:0.72) {$\pi/5$};
  \draw[-{Stealth[length=4pt]},mkc,thick] (18:0.95) arc (18:86:0.95);
  \node[font=\scriptsize,mkc,anchor=west,fill=white,inner sep=1pt] at (52:1.02) {$st$: one click of $\tfrac{2\pi}{5}$};
  \node[font=\scriptsize,align=center,anchor=north] at (0,-1.95)
    {the two reflection axes meet at $\pi/5=36^\circ$,\\so reflecting in both is a rotation by $2\pi/5=72^\circ$};
\end{tikzpicture}}
...
\begin{tikzpicture}[scale=0.75]
  ...
  \node[font=\scriptsize,mkc,anchor=south east] at (4.9,3.15) {the slide $(2,1)$: right 2, up 1};
  \node[font=\scriptsize,anchor=north] at (2,-0.3) {blue arrows: horizontal edges \quad green arrows: vertical edges};
\end{tikzpicture}}
...
\begin{tikzpicture}[scale=0.42,line join=round]
  \foreach \dx/\axis/\cap in {0/face/{through two face\\centres: 3 axes,\\turns by $90^\circ$}, 7.5/vertex/{through two\\corners: 4 axes,\\turns by $120^\circ$}, 15/edge/{through two edge\\midpoints: 6 axes,\\turns by $180^\circ$}} {
    \begin{scope}[xshift=\dx cm]
      \coordinate (A) at (0,0); \coordinate (B) at (3,0); \coordinate (C) at (3,3); \coordinate (D) at (0,3);
      \coordinate (E) at (1.3,1.3); \coordinate (F) at (4.3,1.3); \coordinate (G) at (4.3,4.3); \coordinate (H) at (1.3,4.3);
      \draw[thick] (A)--(B)--(C)--(D)--cycle; \draw[thick] (B)--(F)--(G)--(C); \draw[thick] (D)--(H)--(G);
      \draw[dashed,gray] (A)--(E)--(F); \draw[dashed,gray] (E)--(H);
      \node[font=\scriptsize,align=center,anchor=north,text width=3.6cm] at (2.15,-1.0) {\cap};
    \end{scope} }
...
\begin{tikzpicture}[scale=1.0,>={Stealth[length=4pt]}]
  ...
  \draw[->,very thick,mkc] (0,0) to[bend left=35] node[above,font=\scriptsize] {1: $a$} (2,0);
  \draw[->,very thick,mkc] (2,0) to[bend left=35] node[below,font=\scriptsize] {2: $a\inv$} (0,0);
  \draw[->,very thick,mkc] (0,0) -- node[above,font=\scriptsize,pos=0.6] {3: $a$} (2,0);
  \draw[->,very thick,mkc] (2,0) to[bend left=35] node[left,font=\scriptsize] {4: $b$} (2,1);
  \draw[->,very thick,mkc] (2,1) to[bend left=35] node[right,font=\scriptsize] {5: $b\inv$} (2,0);
  \draw[->,very thick,mkc] (2,0) -- node[above,font=\scriptsize] {6: $a$} (3,0);
...
\begin{tikzpicture}[>={Stealth[length=4pt]},scale=0.9]
  ...
  \draw[rounded corners=6pt,big,dashed] (-0.9,0.35) rectangle (0.4,-1.75); \node[font=\scriptsize,big] at (-0.25,0.6) {kernel $=A_3$};
  \node[font=\scriptsize] at (2,0.9) {$S_3=D_3\ \xrightarrow{\ f\ }\ \Z/2\Z$};
  \node[font=\scriptsize,align=center] at (2,-4.2) {each target point is hit by a whole coset;\\ $S_3/A_3\cong\Z/2\Z$ (first isomorphism theorem)};
\end{tikzpicture}}
...
% ---------- structure for the standalone chapter ----------
\usepackage{titlesec}
\titleformat{\section}{\sffamily\bfseries\Large\color{big}}{\thesection}{0.6em}{}
...
% ---------- figures ----------
\usepackage{caption}
\captionsetup{font=footnotesize,labelfont={sf,bf,color=big},labelsep=quad,
  width=0.95\linewidth,skip=5pt}
\numberwithin{figure}{section}
\setcounter{topnumber}{3}\setcounter{bottomnumber}{3}\setcounter{totalnumber}{6}
\renewcommand{\topfraction}{0.92}\renewcommand{\bottomfraction}{0.75}
\renewcommand{\textfraction}{0.07}\renewcommand{\floatpagefraction}{0.70}
% #1 caption, #2 picture: shrink to the text width only if it must, never magnify,
% so in-figure type prints at the size it was authored at
\newcommand{\sfig}[2]{\begin{figure}[!htb]\centering
  \resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{#2}
  \caption{#1}\end{figure}}
% kept for existing call sites; the first argument is ignored on purpose
\newcommand{\sfigs}[3]{\sfig{#2}{#3}}
\newcommand{\defn}[1]{\textbf{\itshape #1}}
\newtheoremstyle{plainish}{8pt}{8pt}{\itshape}{0pt}{\sffamily\bfseries}{.}{0.6em}{}
\theoremstyle{plainish}
\newtheorem{thm}{Theorem}[section]
\newtheorem{prop}[thm]{Proposition}
\newtheorem{lem}[thm]{Lemma}
\newtheoremstyle{defnish}{8pt}{8pt}{}{0pt}{\sffamily\bfseries}{.}{0.6em}{}
\theoremstyle{defnish}
