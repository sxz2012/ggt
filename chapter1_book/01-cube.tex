...
$A$. The third row attempts a turn by $60^\circ$ and fails. Compare it with the
two rows above, where the cube ends up occupying exactly the space it started in and only the
labels have moved; here the cube itself has shifted, so no corner lands on a corner.}{\figXdn}
